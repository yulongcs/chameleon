...
const advancedConfigValue: any = {
  ...
  tmpFormValue: {},
  getEditingValue: () => {
    return {...advancedConfigValue.tmpFormValue, ...(advancedConfigValue.formRef?.getFieldsValue(FormType.Props) || {})}
  },
}
...
let tmpCurrentNode: TreeNodeProps | any = {}
  ...
  return (
    ...
    <div className={`${typeTest(item?.type) ? style.textArea : ''}`}>
      ...
      {/* <div className={style.magnify}>
          <Magnify item={item} node={tmpCurrentNode} />
        </div> */}
      {canUseConfig && (
        <>
          {/* <Magnify item={item} node={tmpCurrentNode} /> */}
...
export const EmpConfigForm = (props: EmpConfigFormProps) => {
  const {
    id,
    items,
  ...
  useEffect(() => {
    treeStore.formRefMap[formType] = treeStore.formRefMap[formType] || {}
    treeStore.formRefMap[formType][id] = configFormRef
  }, [configFormRef, id, formType])
    ...
    if (canUseConfigRef.current) {
      advancedConfigValue.updateForm = (values: Record<string, any>) => {
        updateInitValues({
          ...
          ...advancedConfigValue.formRef.getFieldsValue(FormType.Props),
          ...values,
        })
      }
    }
          ...
          group?.items?.map((item, index) => {
            return (
              <RenderFormItem
                ...
                id={id}
                item={{...item, aliasName: item.name, index, id}}
                setItems={setItems}
